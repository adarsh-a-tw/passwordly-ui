...
class LoginScreen extends StatefulWidget {
  const LoginScreen({super.key, this.showSessionExpiredMessage = false});
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.showSessionExpiredMessage) {
        PasswordlyAlertDialog.show(
            "Session Expired", "Please relogin!", context);
      }
    });
  }
                ...
                TextFormField(
                    ...
                    validator: (value) {
                      return _isPasswordValid
                          ? null
                          : "Password should'nt be empty.";
                    }),
                const Spacer(),
                PrimaryButton(
                  title: "Login",
                  onPressed:
                      _isUsernameValid && _isPasswordValid ? _submit : null,
                )
              ],
            ),
          );
        ...
        listener: (context, state) async {
          if (state is Authenticated) {
            ...
            await PasswordlyAlertDialog.show(
                "Login Error", state.message, context);
            if (context.mounted) {
